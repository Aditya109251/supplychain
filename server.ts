...
const simulationInterval = setInterval(() => {
  ...
  simulationState.hubs = simulationState.hubs.map(hub => {
    ...
    if (nextStatus === 'red' && hub.status !== 'red') {
      simulationState.logs.unshift({ timestamp: new Date(), message: `CRITICAL: ${hub.name} exceeded 90% capacity.`, type: "error" });
    } else if (nextStatus === 'yellow' && hub.status === 'green') {
      simulationState.logs.unshift({ timestamp: new Date(), message: `WARNING: ${hub.name} high-load state (>70%).`, type: "warning" });
    }
...
// API Routes
app.get("/api/state", (req, res) => res.json(simulationState));
...
app.post("/api/auth/login", async (req, res) => {
  ...
  if (!pool) return res.status(500).json({ error: "DB not connected" });
  try {
    const result = await pool.query('SELECT email, role FROM users WHERE email = $1 AND password = $2', [email, password]);
...
async function setupApp() {
  ...
  // Only listen if explicitly desired or not on Vercel
  if (process.env.VITE_STANDALONE || process.env.NODE_ENV !== "production") {
